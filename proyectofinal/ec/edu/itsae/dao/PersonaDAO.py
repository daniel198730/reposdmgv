...
class PersonaDAO(DBcon.DBcon):
    ...
    def buscarPersonaNombre(self, datobuscado):
        con=self.conexion().connect().cursor()   
        con.execute(""" Select CONCAT(nombre,' ',apell_paterno,' ', apell_materno) as value, idpersona as id from personas where upper(CONCAT(nombre,' ',apell_paterno,' ', apell_materno)) like upper('%s') left join trabajador CONCAT(usuario,' ' ,clave)like upper('%s')"""  % ("%"+datobuscado+"%"))
        reporte=con.fetchall()
        columna=('value', 'id')
        lista=[]
    ...
    def buscarPersonaDato(self, datobuscado):
        con=self.conexion().connect().cursor()   
        sql=""" Select * from personas where upper(CONCAT(nombre,' ',apell_paterno,' ', apell_materno))
